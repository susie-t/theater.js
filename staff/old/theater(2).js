...
Theater.prototype = {
        ...
        if(hash.size() == 1){
          ...
          switch(key){
            ...
            case "setVal":
              ...
              this.setVal(ary, $HH(value).values()[0]);
              this.keep();
              this.result = null;
              break;
            case "branch":
  ...
  translate: function(str){
    ...
    return str.replace(/(\$|\$I(?:MG)?|\$R(?:ESULT)?)?\{([^}]*)\}/g, function(){
      var args = arguments;
      ...
      switch(args[1]){
        ...
        case "$":
          ...
          return this.getVal.apply(this, ary);
          break;
        case "$I":
        case "$IMG":
          var ary = args[2].split("\.");
  ...
  act: function(options){
    ...
    if(options.msg != null){
      this.typist.type({
        text: this.translate(options.msg),
  ...
  menu: function (options){
    ...
    new Theater.Menu({
      ...
      contents: this.translate(options.contents),
  ...
  prompt: function(msg, value){
    ...
    this.menu({
      contents: msg
              + "<input type='text' name='IN' value='" + value + "'/><br/>"
  ...
  choice: function(choices){
    var _theater = this;
    var contents = choices.inject("", function(memo, value){
      return memo + "<button name='" + value + "' class='menu-contents-control'"
      + " style='width:80%;'>" + value + "<\/button><br/>";
    });
    var actions = choices.inject({}, function(memo, value){
      memo[value] = function(menu){
        _theater.setResult(value);
        menu.close();
      };
      return memo;
    });
  ...
  delVal: function(){
    ...
    if(object != null && object[last] != null) delete object[last];
  },
  
  effect: function(inEffect, inOption){
  ...
  loop: function(condition, scenario){
    var init, keep, each;
    var count;
...
Theater.ImageChanger.prototype = {
  ...
  change: function (options) {
    ...
    if(trans){
    ...
    }else if(options.outEffect){
      var outEffect = (options.outEffect instanceof Array) ?
        options.outEffect[0] : options.outEffect;
      var outOption = (options.outEffect instanceof Array) ?
        options.outEffect[1] : {};
      var inEffect, inOption;
      if(options.inEffect){
        if(options.inEffect instanceof Array){
          inEffect = options.inEffect[0];
          inOption = options.inEffect[1];
        }else{
      ...
      Theater.Timer.setTimeout(timerEffect, _show,
...
Theater.PhotoFrame.prototype = {
  initialize: function(options){
    ...
    var _onload = function(){
      ...
      var img = this.img = $(imgId);
      var progress = this.progress = $(progressId);
...
Theater.Menu.prototype = {
  initialize: function(options){
    ...
    $HH(this.options.actions).each(function(pair){
      var elem = this.contents.elements[pair.key];
      if(elem){
